...
| 필드 | 한글명 | 의미 |
|---|---|---|
| **`nrcvb_buy_amt`** | 미수 없는 매수 가능 금액 | 미수(차입) 없이 살 수 있는 금액. 보수적 기준. ISA 응답에 존재. |
| **`ruse_psbl_amt`** | 재사용가능금액 | 매도 대금 등 재사용으로 추가 매수에 쓸 수 있는 금액. |
| **`ord_psbl_cash`** | 주문가능현금 | 즉시 주문에 쓸 수 있는 현금 (퇴직연금 응답에 존재). |
| `psbl_qty_calc_unpr` | 가능수량계산단가 | 가능 수량 산정에 쓰이는 단가 (시장가일 때 KIS 내부 추정). |
| `max_buy_amt` | 최대매수금액 | 위 항목 종합 최대치. |
| `max_buy_qty` | 최대매수수량 | 그 금액으로 살 수 있는 수량. |
